...
class CorrespondenceTagGenerator(QDialog, ui.file_tag_generator.Ui_Form):
    ...
    def set_document_data(self):
        ...
        if self.doc_data[1] == 'Wychodząca':
            self.lbl_caption.setText(f'▶ {self.doc_data[3]} | {self.doc_data[4]} | {self.doc_data[2]}')
        else:
            self.lbl_caption.setText(
                f'◀ {self.doc_data[3]} | ({self.doc_data[5]}) | {self.doc_data[0]} | {self.doc_data[4]} | {self.doc_data[2]}')

        self.doc_data[4] = self.doc_data[4].replace('/', '_').strip()
        self.doc_data[4] = self.doc_data[4].replace('\\', '_').strip()
        # self.doc_data[4] = self.doc_data[4].replace(' ', '_').strip()
        self.doc_data[3] = self.doc_data[3].strip()
        self.doc_data[2] = self.doc_data[2].replace(',', '').strip()

        if self.doc_data[1] == 'Wychodząca':
            doc_file_tag = f'{self.doc_data[3]} {self.doc_data[4]}▶{self.doc_data[2]}.pdf'
        else:
            doc_file_tag = f'{self.doc_data[3]} ({self.doc_data[5]}) {self.doc_data[0]}◀{self.doc_data[4]} {self.doc_data[2]}.pdf'
