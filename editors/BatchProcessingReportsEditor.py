...
import ui.batch_report_editor
from db import ColumnNames, Query
from utils import file_io, info_boxes, delegates, styles

class BatchReportsEditor(QDialog, ui.batch_report_editor.Ui_Form):
    def __init__(self, dbase, manager):
        self.column_titles = ColumnNames.reports_table_column_titles
        self.db = dbase
        self.manager = manager
        self.path_reports = manager.settings[3]
        self.user = manager.settings[4]
        ...
        self.model.setTable("tblBatchReports")
        self.model.select()
        ...
        self.cboFormat.addItems(['html', 'xls', 'csv'])
        ...
        self.btnAddList.clicked.connect(self.add_file)

        self.mapper = QDataWidgetMapper()
        self.mapper.setModel(self.model)
        self.mapper.setSubmitPolicy(QDataWidgetMapper.ManualSubmit)
        self.mapper.addMapping(self.txtName, self.model.fieldIndex('reportName'))
        self.mapper.addMapping(self.txtDefinition, self.model.fieldIndex('reportDefinition'))
        self.mapper.addMapping(self.txtDescription, self.model.fieldIndex('reportDescription'))
        self.mapper.addMapping(self.cboFormat, self.model.fieldIndex('reportOutput'))
        self.new_index = None
        self.mapper.toFirst()
    ...
    def set_edit_controls(self, isEnabled):
        self.txtName.setReadOnly(isEnabled)
        self.txtDefinition.setReadOnly(isEnabled)
        self.txtDescription.setReadOnly(isEnabled)
        self.cboFormat.setEnabled(not isEnabled)

    def clear_controls(self):
        self.txtName.clear()
        self.txtDefinition.clear()
        self.txtDescription.clear()
        self.cboFormat.setCurrentIndex(-1)
        self.txtName.setFocus()

    def on_submit_clicked(self):
        if self.validate_form([self.txtName, self.txtDescription, self.txtDefinition, self.cboFormat]):
            ...
            if self.mapper.submit():
                ...
                if not self.new_index is None:
                    self.tbl_reports.selectionModel().setCurrentIndex(self.new_index,
    ...
    def on_print_clicked(self):
        content_data = []
        output = self.cboFormat.currentText()
        sql_command = self.txtDefinition.toPlainText()
        ...
        for element in self.batch_content:
            result = Query.get_rows(self.db, sql_command.replace('?', '' + element + ''), columns_name)  # each record is a dict in the list
            if not result: continue
            for item in result:
                content_data.append(item) #copy each record to the new list

        if output == 'html':
            self.html_output(content_data)
        if output == 'csv':
            self.csv_output(content_data)
        if output == 'xls':
            self.xls_output(content_data)
        ...
        for row in range(row_count):
            ...
            for item in data[row].items():
                if item[0] == 'Teczka' or item[0] == 'Segment':
                    ...
                    td = item[1]
                    if item[1] == 'Wykonany':
                        td = item[1].replace(item[1],
                                             '<span style="color: darkblue"><strong>' + item[1] + '</strong></span>')
                    ...
                    if item[1].startswith("https://"):
                        td = item[1].replace(item[1],
                                             f'<a href="{item[1]}" target="_blank" rel="noopener noreferrer">Pokaż na mapie</a>')
                        table.append(f'<td>{td}</td>\n')
                    else:
    ...
    def xls_output(self, data):
        try:
            ...
            writer = pandas.ExcelWriter(
                self.path_reports + '\\' + self.user + '_' + report_name + '_' + self.current_date + '.xlsx',
                engine='xlsxwriter')
    ...
    def add_file(self):
        ...
        self.batch_content = file_io.read_line(name) # List of lines
        self.batch_content.sort()
        if not self.batch_content is None:
            self.btn_print.setEnabled(True)
        if len(self.txtPath.text()) == 0:
            self.btn_print.setEnabled(False)
